...
const username = (argv.username || process.env.WEBDAV_CLI_USERNAME || Math.random().toString(36).slice(-8)).toString();
const password = (argv.password || process.env.WEBDAV_CLI_PASSWORD || Math.random().toString(36).slice(-8)).toString();
